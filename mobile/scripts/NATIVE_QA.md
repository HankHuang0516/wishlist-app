...
`--reconfirm-one` 在 `--publish-one` 完整路徑中額外驗證賣家逐件確認之後按「重試 AI」：第一件勾選須立即清除、刊登候選變成 0、公開清單仍為 0；賣家再次勾選後候選才恢復為 1，最後只公開第一件，第二件仍私有。320×640 小螢幕的第 2 件 AI 參考價與商品名稱、卡片確認與頁尾刊登按鈕各自捲動核對，不要求它們同屏出現。2026-09-26 的通過收據 `mobile/build/android-listing-ai-8a7362e5-5e4e-420f-b1e7-6686a8b636e6/result.json` 含 10 張原生截圖及 0 殘留清理；前兩次失敗收據分別保留於 `mobile/build/android-listing-ai-ee15e0eb-0311-4663-8960-74253cca8966/result.json` 與 `mobile/build/android-listing-ai-173091fe-e3aa-4e58-8a0c-4b69b048a26e/result.json`，是測試器的同屏假設與頁尾定位錯誤，不得當成通過。這是既有獨立 Debug 外殼＋當前 Metro JS／隔離資料庫，**不是**新編商店包或 iOS 真機證據。
