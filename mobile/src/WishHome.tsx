...
export function WishHome({ api, apiUrl, onExplore, onWishes }: { api: ReturnType<typeof createApi>; apiUrl: string; onExplore: (wishId?: number, listing?: PublicListing) => void; onWishes: () => void }) {
  ...
  async function match(wishId: number) {
    ...
    try {
      const page = parseWishMatchPage(await api<unknown>(wishMatchPath(wishId, emptySearchFilters, TAIWAN_BOUNDS)), wishId, apiUrl, __DEV__);
  ...
  return <ScrollView contentContainerStyle={s.content}><Text style={s.title}>讓願望更靠近。</Text><Text style={s.body}>願望先行，地圖幫你發現下一個好物。</Text>
    ...
    {!matching && !matchError && !!wish && <Text style={s.heading}>{visible.length ? '這一頁的願望線索' : '目前沒有吻合商品'}</Text>}
    {visible.slice(0, 5).map(m => <Pressable key={m.listing.id} accessibilityRole="button" onPress={() => onExplore(m.wishItemId, m.listing)} style={s.card}><View style={s.row}><Image source={{ uri: m.listing.media[0].thumbnailUrl }} accessibilityLabel={m.listing.title} style={s.image} /><View style={s.flex}><Text style={s.heading}>{m.listing.title}</Text><Text style={s.body}>{listingPrice(m.listing)}</Text><Text style={s.small}>{m.listing.location.county} · {m.listing.location.district}</Text></View></View><Text style={s.small}>本頁吻合分數 {m.score}／100</Text>{m.reasons.map(r => <Text key={r.code} style={s.small}>{r.text}</Text>)}</Pressable>)}
