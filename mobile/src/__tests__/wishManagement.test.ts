...
const wish = { id: 4, wishlistId: 1, name: 'Sony', notes: null, link: null, maxPrice: null, priceCurrency: null, isHidden: false, isPurchased: false };
const form = { name: ' Sony ', notes: '', link: '', budget: '', currency: 'TWD' };
const clientRequestId = 'b5abf861-a66d-4072-876b-4f0ab3172dac';
describe('native wish management boundaries', () => {
  it('keeps minimal own management DTOs and strips unrelated fields', () => { expect(parseManagedList({ ...list, apiKey: 'synthetic-only' })).toEqual({ id: 1, title: 'Camera', description: null, isPublic: false, maxItems: 100, count: 1 }); expect(parseManagedWish({ ...wish, proxy_end_user_id: 'synthetic-only' })).toEqual(wish); });
  ...
  it('distinguishes an omitted budget from a zero budget and normalizes currencies', () => { expect(wishDraftBody(form)).toEqual({ name: 'Sony', notes: null, link: null, maxPrice: null }); expect(wishDraftBody({ ...form, budget: '0', currency: 'usd', notes: 'notes' })).toMatchObject({ maxPrice: 0, priceCurrency: 'USD', notes: 'notes' }); });
