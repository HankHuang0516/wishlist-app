const { destinationTestRun, anonymousSummaryPassed, iosSummaryPassed, TESTS,
    ...
    AUTHENTICATED_LISTING_TWO_PHOTO_TESTS, AUTHENTICATED_EXTERNAL_MAP_TESTS } = require('../../../mobile/scripts/ios-xctestrun-config.cjs');
...
describe('supervised destination-only anonymous Xcode tests', () => {
    ...
    it('selects only the isolated external-source map check', () => {
        const qa = destinationTestRun(template(), label, '/products', 23456, 'external-map').WishlistNativeQa;
        expect(qa.OnlyTestIdentifiers).toEqual(AUTHENTICATED_EXTERNAL_MAP_TESTS);
    });
    ...
    it.each(['marketplace-discovery', 'marketplace-chat', 'marketplace-meetup', 'deletion', 'listing-batch-entry', 'listing-batch-photo', 'listing-batch-two-photos', 'external-map'])('rejects anonymous execution with authenticated flow %s', flow => {
