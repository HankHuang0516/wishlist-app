...
export function mediaError(res: Response, error: unknown) {
    ...
    if (error instanceof MediaStorageConfigurationError) return res.status(503).json({ error: '商品照片儲存尚未配置，請稍後再試', errorCode: 'PHOTO_STORAGE_UNAVAILABLE' });
...
export async function uploadListingMedia(req: AuthRequest, res: Response) {
    ...
    try {
        ...
        id = randomUUID();
        await storage.write(id, photo.image, photo.thumbnail);
        persisted = true;
        ...
        try {
            const record = await prisma.listingMedia.create({ data: { id, ownerUserId, clientUploadId, contentHash: photo.contentHash,
                width: photo.width, height: photo.height, byteSize: photo.byteSize, imageUrl: `${base}/image`, thumbnailUrl: `${base}/thumbnail` }, select });
            return res.status(201).json(record);
        } catch (error) {
            await storage.remove(id).catch(() => console.error('Private photo rollback cleanup needs retry; details withheld'));
            persisted = false;
        ...
        if (id && persisted) await storage.remove(id).catch(() => console.error('Private photo rollback cleanup needs retry; details withheld'));
