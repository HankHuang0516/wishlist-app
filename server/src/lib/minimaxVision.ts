...
export type MinimaxVisionConfig = {
    apiKey: string;
    apiHost: string;
...
export function loadMinimaxVisionConfig(env: NodeJS.ProcessEnv = process.env): MinimaxVisionConfig | null {
    const apiKey = env.MINIMAX_API_KEY?.trim();
    if (!apiKey) return null;
    const apiHost = (env.MINIMAX_API_HOST || 'https://api.minimax.io').replace(/\/$/, '');
    if (!['https://api.minimax.io', 'https://api.minimaxi.com'].includes(apiHost)) {
        throw new Error('MINIMAX_API_HOST must be an official MiniMax API host');
    }
    const hosts = [
        env.RAILWAY_PUBLIC_DOMAIN,
        env.RAILWAY_STATIC_URL,
        ...(env.MINIMAX_VISION_IMAGE_HOSTS || '').split(','),
    ].filter(Boolean).map(value => {
        try { return new URL(value!.startsWith('http') ? value! : `https://${value}`).hostname.toLowerCase(); }
        catch { throw new Error('MINIMAX_VISION_IMAGE_HOSTS contained an invalid host'); }
    });
    return { apiKey, apiHost, allowedImageHosts: [...new Set(hosts)] };
}
...
export function parseVisionEvidence(content: string): VisionEvidence {
    ...
    return {
        ...
        evidence,
        uncertainties: strings(raw.uncertainties, 4, 120),
        confidence,
    };
}
...
export function buildVisionPrompt() {
    return [
        ...
        '先辨識畫面主體，再逐項記錄支持判斷的可見證據。招牌／海報上的清晰文字可以識別食物或商品，但要指出它是文字證據；多件商品則用群組名稱，不要憑空挑一件。',
        '尤其不可把公仔／玩具推論成電腦，也不可把食品招牌推論成相機。看不到型號、品牌、新舊或價格時填 null；不要估價，價格由下一階段處理。',
...
async function readBoundedImage(url: string, config: MinimaxVisionConfig, fetchImpl: Fetch): Promise<Buffer> {
...
export async function understandImage(url: string, config: MinimaxVisionConfig, fetchImpl: Fetch = fetch): Promise<VisionEvidence> {
    ...
    timer.unref?.();
    try {
        const response = await fetchImpl(`${config.apiHost}/v1/coding_plan/vlm`, {
            method: 'POST', signal: controller.signal,
            headers: { authorization: `Bearer ${config.apiKey}`, 'content-type': 'application/json', 'MM-API-Source': 'Wishlist-AI' },
            body: JSON.stringify({ prompt: buildVisionPrompt(), image_url: `data:image/webp;base64,${prepared.toString('base64')}` }),
        });
        if (!response.ok) throw new VisionError('MiniMax vision request failed', 'VISION_UPSTREAM');
        const data = await response.json() as any;
        if (data?.base_resp?.status_code !== 0 || typeof data.content !== 'string') {
            throw new VisionError('MiniMax vision response was invalid', 'VISION_UPSTREAM');
        }
        return parseVisionEvidence(data.content);
    } catch (error: any) {
        if (error instanceof VisionError) throw error;
        throw new VisionError('MiniMax vision request failed', controller.signal.aborted ? 'VISION_TIMEOUT' : 'VISION_NETWORK');
    } finally { clearTimeout(timer); }
}
